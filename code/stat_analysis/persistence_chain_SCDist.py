...
def populate_persgraph(graph,coefs,S,C):
  ...
  minx=int(mot_ranges['chain'][0]*100)
  maxx=int(mot_ranges['chain'][1]*100)
  ...
  # for dist in [0.5,0.42,0.34,0.26,0.18,0.1]:
    ...
    for x in range(minx,maxx):
      scalx=(float(x)/100-scales['prop_chain'][0])/scales['prop_chain'][1]
      y=coefs[1]['coef'] # Global intercept
      y+=coefs[2]['coef']*scalx # Chain
      y+=coefs[3]['coef']*scalDist # Disturbance
      y+=coefs[4]['coef']*scalC # Connectance
      y+=coefs[5]['coef']*scalS # Size
      y+=coefs[6]['coef']*scalDist*scalx # Disturbance, chain
      y+=coefs[7]['coef']*scalDist*scalS # Disturbance, size
...
grace.set_row_xaxislabel(row=1,colspan=(None,None),label='Proportion of three-species chain motif in role',char_size=1,perpendicular_offset=.05)
grace.set_col_yaxislabel(col=0,rowspan=(None,None),label='Probability of persistence',char_size=1,perpendicular_offset=.05)

grace.write_file('../../manuscript/figures/persistence_chain_detailpers.eps')
