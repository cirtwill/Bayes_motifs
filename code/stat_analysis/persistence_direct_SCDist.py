...
def populate_persgraph(graph,coefs,S,C):
  ...
  # for dist in [0.5,0.42,0.34,0.26,0.18,0.1]:
    ...
    for x in range(minx,maxx):
      ...
      y+=coefs[2]['coef']*scalx # Direct
      y+=coefs[3]['coef']*scalS # Size
      y+=coefs[4]['coef']*scalC # Connectance
      y+=coefs[5]['coef']*scalDist # Disturbance
      y+=coefs[6]['coef']*scalS*scalx # Direct, Size
      y+=coefs[7]['coef']*scalx*scalC # Direct, Connectance
      y+=coefs[8]['coef']*scalS*scalC # Connectance, Size
      y+=coefs[9]['coef']*scalDist*scalS # Disturbance, size
      y+=coefs[10]['coef']*scalDist*scalC # Connectance, Disturbance
      y+=coefs[11]['coef']*scalC*scalS*scalx # Direct, Size, Connectance 
...
grace.set_row_xaxislabel(row=1,colspan=(None,None),label='Proportion of direct competition motif competition in role',char_size=1,perpendicular_offset=.05)
grace.set_col_yaxislabel(col=0,rowspan=(None,None),label='Probability of persistence',char_size=1,perpendicular_offset=.05)

grace.write_file('../../manuscript/figures/persistence_direct_detailpers.eps')
