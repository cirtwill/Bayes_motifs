...
def populate_persgraph(graph,coefs,S,C):
  ...
  minx=int(mot_ranges['omnivory'][0]*100)
  maxx=int(mot_ranges['omnivory'][1]*100)
...
#    print(coefs[14])
    for x in range(minx,maxx):
      scalx=(float(x)/100-scales['prop_omni'][0])/scales['prop_omni'][1]
      y=coefs[1]['coef'] # Global intercept
      y+=coefs[2]['coef']*scalx # Omni
      y+=coefs[3]['coef']*scalDist # Disturbance
      y+=coefs[4]['coef']*scalS # Size
      y+=coefs[5]['coef']*scalC # Connectance
      y+=coefs[6]['coef']*scalDist*scalx # Disturbance, omni
      y+=coefs[7]['coef']*scalx*scalS # omni, Size
      y+=coefs[8]['coef']*scalDist*scalS # Disturbance, Size
      y+=coefs[9]['coef']*scalx*scalC # Omni, Connectance
      y+=coefs[10]['coef']*scalDist*scalC # Disturbance, Connectance
      y+=coefs[11]['coef']*scalS*scalC # Size, Connectance, 
      logity=math.exp(y)/(1+math.exp(y))
      y+=coefs[12]['coef']*scalx*scalDist*scalC # Disturbance, Connectance, omni
      y+=coefs[13]['coef']*scalx*scalS*scalC # Size, Connectance, omni
...
grace.graphs[0].add_drawing_object(DrawText,x=0.67,y=0.67,loctype='world',text='Basal species',just=0,char_size=.5)
grace.graphs[0].add_drawing_object(DrawText,x=0.67,y=0.62,loctype='world',text='extinction',just=0,char_size=.5)
grace.graphs[0].add_drawing_object(DrawText,x=0.67,y=0.57,loctype='world',text='probability',just=0,char_size=.5)
grace.graphs[0].legend.configure(char_size=.5,loc=(0.65,0.55),loctype='world',box_linestyle=0,fill_pattern=0)
...
grace.set_row_xaxislabel(row=1,colspan=(None,None),label='Proportion of omnivory motif in role',char_size=1,perpendicular_offset=.05)
grace.set_col_yaxislabel(col=0,rowspan=(None,None),label='Probability of persistence',char_size=1,perpendicular_offset=.05)

grace.write_file('../../manuscript/figures/persistence_omnivory_detailpers.eps')
