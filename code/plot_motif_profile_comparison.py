...
def format_graph(graph,xtype,ytype):
  ...
  graph.xaxis.label.configure(text='Frequency of '+xtype,char_size=.75,just=2,place='normal')
  ...
  if ytype=='omnivory':
    ...
    graph.world.ymin=0.0999999999999
  graph.world.ymax=.340000001
  graph.yaxis.tick.major=.1
  graph.yaxis.label.configure(text='Frequency of '+str.capitalize(ytype),char_size=.75,just=2,place='normal')
...
def populate_graph(graph,xtype,ytype,sim_profiles,sim_info,emp_profiles,emp_info):
  ...
  for ID in emp_profiles[xtype][ytype]:
    ...
    if ytype=='omnivory':
      ...
      y=emp_profiles[xtype][ytype][ID][0][1]+.008,loctype='world',just=2)
    else:
      graph.add_drawing_object(DrawText,text=str.capitalize(ID.split('_')[0]),char_size=.55,x=emp_profiles[xtype][ytype][ID][0][0],
      y=emp_profiles[xtype][ytype][ID][0][1]+.005,loctype='world',just=2)
