...
%DIF DEL reorder_main_text.tex         Fri Jun 17 15:02:58 2022
%DIF ADD PRSB_initial_submission.tex   Thu Feb 24 14:37:41 2022
\usepackage{amsmath} 
...
%DIF < New
    ...
    %\subsection*{1 Why are bottom-up effects important for understanding stability?}
     The plant community is the foundation upon which the myriad species in \DIFaddbegin \DIFadd{terrestrial }\DIFaddend food webs depend for their survival. Disturbances \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the plant community have been shown to affect important ecosystem properties such as primary \citep{Hector1999} and secondary production \citep{borer2012plant}, soil respiration and carbon cycling \citep{chen2019plant} and consumer diversity \citep{scherber2010bottom, Baiser2016}. \DIFdelbegin \DIFdel{As such, the structure }\DIFdelend \DIFaddbegin \DIFadd{The stability }\DIFaddend of the plant community \DIFdelbegin \DIFdel{affects stability across all levels of ecosystem organization, which }\DIFdelend is crucial for sustaining a healthy energy flow in the ecosystem as a whole \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{proulx2010diversity, scherber2010bottom, Rosenblatt2016}}\hspace{0pt}%DIFAUXCMD
. 
     For example, the loss }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Rosenblatt2016} }\hspace{0pt}%DIFAUXCMD
and the structure of the plant community affects stability across all levels of ecosystem organization \mbox{%DIFAUXCMD
\citep{proulx2010diversity,scherber2010bottom}}\hspace{0pt}%DIFAUXCMD
. Loss }\DIFaddend of diversity at the basal level typically causes declines in both abundance and richness of all types of consumers: herbivores, predators, parasitoids, etc. \citep{scherber2010bottom}.

     %DIF < \subsection*{2 Global and local network properties are known to affect stability}
    \DIFdelbegin \DIFdel{How disturbances, such as }\DIFdelend %DIF > \subsection*{2 Bayesian networks are a fantastic way to model bottom-up effects}
    \DIFaddbegin \DIFadd{How }\DIFaddend population decline or extinction \DIFdelbegin \DIFdel{, lead }\DIFdelend \DIFaddbegin \DIFadd{of one species leads }\DIFaddend to loss of other species via direct and indirect effects (i.e., secondary extinctions) \DIFdelbegin \DIFdel{has long been }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend a vibrant area of research \citep{Santos2021,curtsdotter2011robustness, dunne2009cascading, Eklof2006}. \DIFdelbegin \DIFdel{Several global (whole-network) as well as local (single-species) properties are known to influence how species respond to disturbances. Two well-known, influential global properties are network size, the number of species present }\DIFdelend \DIFaddbegin \DIFadd{Traditionally, there are two main approaches for studying secondary extinctions. First, there are topological models, based only on food web structure \mbox{%DIFAUXCMD
\citep{dunne2009cascading}}\hspace{0pt}%DIFAUXCMD
. Here, extinctions only affect other species in the network once a consumer has lost all of its prey and therefore must go extinct. The second approach uses dynamical models which explicitly simulate population dynamics using a system of differential equations \mbox{%DIFAUXCMD
\citep{binzer2011susceptibility}}\hspace{0pt}%DIFAUXCMD
. Dynamical models take changes in prey or predator densities into account when calculating the densities of other species }\DIFaddend in the network\DIFaddbegin \DIFadd{. 
    ...
    \DIFadd{A middle‐ground approach for simulating secondary extinctions is to use Bayesian networks \mbox{%DIFAUXCMD
\citep{Eklof2013}}\hspace{0pt}%DIFAUXCMD
. 
    In this framework, a consumer's probability of extinction depends on the fraction of resources lost and on a baseline probability of extinction which captures causes unrelated to the network structure (e.g.}\DIFaddend , \DIFdelbegin \DIFdel{and network connectance, the number of realized feeding interactions.
    Based on simulations of random networks, large networks were initially thought to be less stable~\mbox{%DIFAUXCMD
\citep{May1972}}\hspace{0pt}%DIFAUXCMD
.The indisputable presence of large, stable, empirical networks has spurred research into how other structural characteristics, including connectance, may allow large numbers of species to persist~\mbox{%DIFAUXCMD
\citep{Dunne2002}}\hspace{0pt}%DIFAUXCMD
. 
    Highly-connected networks may be more resistant to secondary extinctions because consumers , on average, have access to a larger number of prey species and can switch from a declining or extinct prey to another resource~\mbox{%DIFAUXCMD
\citep{Dunne2002, Eklof2006,Baumgartner2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{disease, stochastic extinction of small populations).
    Modelling Bayesian networks is fast and computationally efficient, allowing analysis of networks with high species richness \mbox{%DIFAUXCMD
\citep{Haussler2020}}\hspace{0pt}%DIFAUXCMD
. 
    As consumers do not affect the extinction probabilities of their resources, Bayesian networks can only capture bottom-up effects of disturbances within networks}\DIFaddend . 
    However, \DIFdelbegin \DIFdel{a well-connected network also includes more pathways through which a disturbance can spread~\mbox{%DIFAUXCMD
\citep{Dunne2004, Vieira2015}}\hspace{0pt}%DIFAUXCMD
.
    Depending on the strength and arrangements of interactions, this means that species may be impacted through several paths, amplifying the original disturbanceand causing more secondary extinctions.
    The net effect of connectance on extinction therefore likely depends on finer-scale (species-level) network structure, as well as the type and severity of disturbance}\DIFdelend \DIFaddbegin \DIFadd{Bayesian networks do capture the majority of secondary extinctions captured by dynamical models \mbox{%DIFAUXCMD
\citep{Eklof2013}}\hspace{0pt}%DIFAUXCMD
.
    ...
    %DIF > \subsection*{3 Global scale network properties are known to affect stability}
    \DIFadd{Using Bayesian networks, researchers can explore how the structures of realistically-large networks shape their responses to disturbance.
    The global (whole-network) structural properties of ecological networks have implications for their functioning \mbox{%DIFAUXCMD
\citep{Petchey2002}}\hspace{0pt}%DIFAUXCMD
, stability \mbox{%DIFAUXCMD
\citep{Allesina2012} }\hspace{0pt}%DIFAUXCMD
and robustness to disturbances \mbox{%DIFAUXCMD
\citep{Dunne2002, Eklof2006}}\hspace{0pt}%DIFAUXCMD
. 
    For example, more highly-connected networks are more resistant to secondary extinctions \mbox{%DIFAUXCMD
\citep{Dunne2002, Eklof2006} }\hspace{0pt}%DIFAUXCMD
and a modular organization, where some groups of species are tightly interconnected with limited connections between groups, can limit the propagation of disturbances \mbox{%DIFAUXCMD
\citep{Krause2003, Teng2004}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

    \DIFdelbegin \DIFdel{Species-level properties known to affect species' responses to disturbances include trophic level (a measure of the path length between a consumer and a basal resource) and in-degree (number of prey items of a predator) . 
    These can also be referred to as local properties of the network}\DIFdelend %DIF > \subsection*{4 meso-scale nw propoerties also affect stability}
    \DIFaddbegin \DIFadd{Structural properties on the local (single-species) scale are also known to affect response to disturbance}\DIFaddend . 
    Species with high trophic levels (long paths to basal resources)\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend low in-degree (few prey)\DIFaddbegin \DIFadd{, and low abundances }\DIFaddend are generally at higher risks of secondary extinction \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{binzer2011susceptibility, Eklof2006}}\hspace{0pt}%DIFAUXCMD
, and disturbances to plant populations cause particularly many secondary extinctions \mbox{%DIFAUXCMD
\citep{Curtsdotter2011}}\hspace{0pt}%DIFAUXCMD
. 
    %DIF < including secondary extinction due to reduced plant populations.
    }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{binzer2011susceptibility}}\hspace{0pt}%DIFAUXCMD
. 
    }\DIFaddend However, in-degree and trophic level are both relatively coarse descriptions of a species' role \DIFdelbegin \DIFdel{in its food web}\DIFdelend \DIFaddbegin \DIFadd{within its food-web}\DIFaddend ~\citep{Cirtwill2018FoodWebs}. 
    \DIFdelbegin \DIFdel{A more detailed perspective is desirable since, as well as the properties of the consumer itself, a consumer's risk of secondary extinction after a disturbance to plants depends on the extinction risk of its prey. 
    }\DIFdelend For example, a \DIFdelbegin \DIFdel{generalist consuming many specialist (and therefore high-risk) prey }\DIFdelend \DIFaddbegin \DIFadd{specialist may not necessarily have a high risk of extinction if it consumes a basal resource which is unlikely to be affected by disturbance. 
    On the other hand, a consumer with low trophic level }\DIFaddend may be at \DIFdelbegin \DIFdel{greater }\DIFdelend \DIFaddbegin \DIFadd{high }\DIFaddend risk of extinction \DIFdelbegin \DIFdel{than a specialist which consumes a highly generalist resource that is less likely to go extinct (Fig.~\ref{fig:concept})}\DIFdelend \DIFaddbegin \DIFadd{if it consumes prey which are themselves very likely to be affected by disturbance}\DIFaddend . 

    %DIF < \subsection*{3 Motif participation may similarly affect species' particular extinction risk}
    \DIFdelbegin \DIFdel{It is possible to capture information about how a consumer and its prey fit together within a community by defining }\DIFdelend %DIF > \subsection*{5 Motif participation may similarly affect species' particular extinction risk}
    \DIFaddbegin \DIFadd{One way to incorporate more nuance into descriptions of how species fit into their communities is to define species' roles based on their }\DIFaddend participation in different motifs -- unique combinations of small numbers of interacting species~\citep{Stouffer2007,Stouffer2012}. \DIFdelbegin \DIFdel{Such motif profiles of species are easily calculated once we know the network structure and require no more information than is used to calculate degree or trophic level.
    These }\DIFdelend \DIFaddbegin \DIFadd{Importantly, these }\DIFaddend `motif participation' roles capture information about species' direct \DIFdelbegin \DIFdel{as well as }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend indirect interactions, incorporating larger-scale network structure into species-level descriptions~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Cirtwill2015a,Cirtwill2018FoodWebs}}\hspace{0pt}%DIFAUXCMD
.
    ...
    Therefore, by allowing researchers to distinguish between}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Cirtwill2015a}}\hspace{0pt}%DIFAUXCMD
. 
    This means that two species with the same in-degrees and trophic levels can still participate in different sets of motifs if}\DIFaddend , for example, \DIFdelbegin \DIFdel{consumers of specialists and generalists, motif participation may complement degree and trophic level to more fully predict species' extinction risk .
    }%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{While the potential relationships between motif participation and species-level extinction risk have not yet been tested to our knowledge, it has been shown that }\DIFdelend \DIFaddbegin \DIFadd{one interacts with specialists and the other interacts with generalists~\mbox{%DIFAUXCMD
\citep{Cirtwill2018FoodWebs}}\hspace{0pt}%DIFAUXCMD
. 
    This extra detail may be useful when predicting  extinction risk since, at the network level, }\DIFaddend the frequencies of different motifs \DIFdelbegin \DIFdel{within a whole network }\DIFdelend are associated with community stability \citep{prill2005dynamic, bascompte2005simple} and with changes to the composition of plant communities \citep{giling2019plant}. 
    \DIFdelbegin \DIFdel{These relationships between network-level motif frequencies (`motif profiles') and overall community stability mean that a network's motif profile could affect the average extinction risk for consumers within the network.
    Moreover, if network motif profiles are related to average extinction risk, it is worth investigating whether a }\DIFdelend \DIFaddbegin \DIFadd{We therefore expect that both the motif structure of a network and }\DIFaddend species' participation in \DIFdelbegin \DIFdel{different motifs also affects its extinction risk.
}%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Severity of disturbance
    \DIFdel{As mentioned above, the severity of a disturbance may interact with network structure }\DIFdelend \DIFaddbegin \DIFadd{that structure should be related to risks due to disturbance }\DIFaddend to \DIFdelbegin \DIFdel{affect secondary extinctions.
    A mild disturbance (e.g., leading to a small increase in the probability of basal resourcesgoing extinct) is intuitively likely to cause fewer secondary extinctions than a severe disturbance, and the difference in effects can depend on network structure~\mbox{%DIFAUXCMD
\citep{Baumgartner2015}}\hspace{0pt}%DIFAUXCMD
.
    Empirical studies have shown that disturbances to plant species can affect some consumers more strongly than others \mbox{%DIFAUXCMD
\citep{byrnes2011climate}}\hspace{0pt}%DIFAUXCMD
, that the strength of disturbance affects which consumer species are most strongly affected~\mbox{%DIFAUXCMD
\citep{detmer2021variation,carnell2020more}}\hspace{0pt}%DIFAUXCMD
, and that the resilience and return time of the ecological community can be strongly influenced by disturbance severity \mbox{%DIFAUXCMD
\citep{rydgren2004disturbance}}\hspace{0pt}%DIFAUXCMD
. 
    The severity of disturbance therefore clearly has an effect on how the ecological community will respond}\DIFdelend \DIFaddbegin \DIFadd{basal resources}\DIFaddend . 

           %\subsection*{Here we test...}
    \DIFdelbegin \DIFdel{Here}\DIFdelend \DIFaddbegin \DIFadd{Specifically}\DIFaddend , we are interested in whether a consumer's extinction risk \DIFdelbegin \DIFdel{of going secondarily extinct }\DIFdelend depends on 1) the motifs in which \DIFdelbegin \DIFdel{the species participates , }\DIFdelend \DIFaddbegin \DIFadd{a focal species participates and }\DIFaddend 2) the motif profile of the full food web\DIFdelbegin \DIFdel{and 3) the severity of the disturbance to the basal level. 
    %DIF < We address this in scenarios with various levels of disturbance targeting the basal level.
    }\DIFdelend \DIFaddbegin \DIFadd{. 
    }\DIFaddend In addition, we want to understand how the information provided by motifs is related to \DIFdelbegin \DIFdel{other }\DIFdelend \DIFaddbegin \DIFadd{classical }\DIFaddend global and local food web properties in order to better synthesize information provided by different scales of network structure.
    \DIFdelbegin \DIFdel{We use synthetic networks and model secondary extinctions using Bayesian networks \mbox{%DIFAUXCMD
\citep{Eklof2013, Haussler2020}}\hspace{0pt}%DIFAUXCMD
. 
    ...
    We focus our analyse on the four network }\DIFdelend \DIFaddbegin \DIFadd{Of the 13 three-species motifs that can occur in empirical food webs, there are four }\DIFaddend motifs that are over-represented in empirical networks and \DIFdelbegin \DIFdel{additionally have been shown }\DIFdelend \DIFaddbegin \DIFadd{are thought }\DIFaddend to contribute to \DIFdelbegin \DIFdel{network stability : }\DIFdelend \DIFaddbegin \DIFadd{stability \mbox{%DIFAUXCMD
\citep{Stouffer2007, Borrelli2015a, giling2019plant}}\hspace{0pt}%DIFAUXCMD
. These four motifs, }\DIFaddend the three-species chain, omnivory, apparent competition, and direct competition\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Stouffer2007, Borrelli2015a, giling2019plant}}\hspace{0pt}%DIFAUXCMD
.
   }\DIFdelend \DIFaddbegin \DIFadd{, are also the only ones which can occur in the acyclic networks required for Bayesian network simulations~\mbox{%DIFAUXCMD
\citep{Eklof2013}}\hspace{0pt}%DIFAUXCMD
. 
...
\section*{Methods}
	...
	\DIFdelbegin \subsection*{\DIFdel{Modelling secondary extinction using Bayesian networks}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Initial network construction}}
\DIFaddend 

		\DIFdelbegin \DIFdel{Traditionally, there are two main approaches for studying secondary extinctions. 
    First, there are topological models, based only on food web structure \mbox{%DIFAUXCMD
\citep{dunne2009cascading}}\hspace{0pt}%DIFAUXCMD
. 
        ...
        The second approach uses dynamical models which explicitly simulate population dynamics using a system of differential equations \mbox{%DIFAUXCMD
\citep{binzer2011susceptibility}}\hspace{0pt}%DIFAUXCMD
. 
		Dynamical models take changes in prey or predator densities into account when calculating the densities of other species in the network}\DIFdelend \DIFaddbegin \DIFadd{We generated a realistic set of simulated networks based on the niche model~\mbox{%DIFAUXCMD
\citep{Williams2000,Stouffer2007} }\hspace{0pt}%DIFAUXCMD
using the function "nichemodel" within the Julia~\mbox{%DIFAUXCMD
\citep{Bezanson2017julia} }\hspace{0pt}%DIFAUXCMD
package }\emph{\DIFadd{BioEnergeticFoodWebs}}\DIFadd{~\mbox{%DIFAUXCMD
\citep{bioenergfw}}\hspace{0pt}%DIFAUXCMD
. 
		...
		For full details, see~\mbox{%DIFAUXCMD
\citet{Cirtwill2021_inprep}}\hspace{0pt}%DIFAUXCMD
.
		We removed any biologically unlikely networks (Appendix }\emph{\DIFadd{S1}}\DIFadd{) and replaced them with new simulated networks, repeating the process until we obtained 100 suitable networks in each combination of size and connectance.
		All networks were rendered acyclic following~\mbox{%DIFAUXCMD
\citet{Allesina2009functional} }\hspace{0pt}%DIFAUXCMD
and topologically sorted from lowest to highest trophic level (Appendix }\emph{\DIFadd{S1}}\DIFadd{).
	...
	\subsection*{\DIFadd{Calculating persistence}}	
    %DIF >  basic idea behind BN modeling
        \DIFadd{Bayesian networks can be used to represent species and their feeding interactions as the nodes and links of a network (see Appendix }\emph{\DIFadd{S2}} \DIFadd{for full details).
        In our setting, each species (node) is a Bernoulli random variable, being either extant or extinct. 
        The probability that a particular species is extinct is a function of the states of its resources}\DIFaddend .
        \DIFdelbegin \DIFdel{This additional detail means that realistic processes such as indirect interactions can be included, but also means that dynamical models are highly parameter intensive and simulations are much more time-consuming than topological models. 
        }\DIFdelend 

		\DIFdelbegin \DIFdel{A middle‐ground approach for simulating secondary extinctions is to use Bayesian networks \mbox{%DIFAUXCMD
\citep{Eklof2013,Haussler2020}}\hspace{0pt}%DIFAUXCMD
.
        In this framework, a consumer's probability of extinction depends on the fraction of resources lost ($f = k/n$ for a species with $n$ resources, of which $k$ have gone extinct) and on }\DIFdelend %DIF >  probability of extinction equation
		%DIF > Consider a species with $n$ resource species. 
		\DIFaddbegin \DIFadd{Before taking food-web structure into account, all species already have }\DIFaddend a baseline probability of extinction\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $\pi$\DIFdelbegin \DIFdel{) which captures factors affecting the focal species' extinction probability that are not related to the network structure (e.g., disease}\DIFdelend \DIFaddbegin \DIFadd{. 
		$\pi$ represents the risk of going extinct due to diseases}\DIFaddend , stochastic extinction of small populations\DIFdelbegin \DIFdel{).
        Modelling Bayesian networks is fast and computationally efficient, allowing analysis of networks with high species richness \mbox{%DIFAUXCMD
\citep{Haussler2020}}\hspace{0pt}%DIFAUXCMD
. 
        This increased speed allows us to simulate a range of disturbance severities rather than single species removals as in many previous studies (e.g.
		,~\mbox{%DIFAUXCMD
\citealp[]{Memmott2004,Staniczenko2010,Dunne2004}}\hspace{0pt}%DIFAUXCMD
). 
        Moreover, Bayesian network simulations are less parameter-intensive than dynamical models, meaning that they are less sensitive to assumptions made in the modelling process~\mbox{%DIFAUXCMD
\citep{Eklof2013}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, or other factors not related to the food web.
		A species probability of extinction also depends on the state of its resources. 
        If the focal species is a consumer with $n$ resources, of which $k$ have gone extinct, its probability of extinction depends on $\pi$ and on the fraction $f = k/n$}\DIFaddend .

        
        The response of the consumer to changes in $f$ can take any form (\DIFdelbegin %DIFDELCMD < \emph{%%%
...
\DIFaddbegin 
		...
		%DIF <  BN limitations
		Modelling species extinctions in food webs using Bayesian networks has two main limitations. First, \DIFdelbegin \DIFdel{in Bayesian networks consumers do not affect the extinction probabilities of their resources and as such we can here only capture bottom-up effects of disturbances. 
		However, Bayesian networks has been shown to capture the majority of secondary extinctions produced by fully-fledged dynamical models \mbox{%DIFAUXCMD
\citep{Eklof2013}}\hspace{0pt}%DIFAUXCMD
.
		\mbox{%DIFAUXCMD
\cite{Eklof2013} }\hspace{0pt}%DIFAUXCMD
analyzed which functional form of consumers' response to loss of resources most accurately predicted the secondary extinctions realized in the dynamical model. 
		...
		For example, bottom-up effects regulate zooplankton communities in freshwater lakes, regulating zooplankton communities \mbox{%DIFAUXCMD
\citep{li2020bottom} }\hspace{0pt}%DIFAUXCMD
and control abundances and biomass of vertebrate species in several terrestrial systems \mbox{%DIFAUXCMD
\citep{Dobson2009food, Mduma1999food, Georgiadis2007}}\hspace{0pt}%DIFAUXCMD
. 
		Second, }\DIFdelend due to the strict bottom-up approach \DIFdelbegin \DIFdel{of Bayesian networks, the networks }\DIFdelend \DIFaddbegin \DIFadd{the food webs }\DIFaddend must be acyclic. 
		\DIFdelbegin \DIFdel{Fortunately, \mbox{%DIFAUXCMD
\cite{Allesina2009functional} }\hspace{0pt}%DIFAUXCMD
proved that cyclic and acyclic versions of a network have the same properties in terms of secondary extinctions and the effect on network robustness are very limited. 
		...
		Moreover, the cyclic motifs which are excluded from an acyclic network are rare in empirical networks~\mbox{%DIFAUXCMD
\citep{Stouffer2007} }\hspace{0pt}%DIFAUXCMD
and therefore unlikely to strongly affect extinction risk}\DIFdelend \DIFaddbegin \DIFadd{Second, as consumers are influenced by the state of their resources, but not vice versa, the method does not capture top-down effects}\DIFaddend .

    \DIFdelbegin \subsection*{\DIFdel{Disturbance scenarios}}
    %DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD <         %%%
\DIFdelend %DIF >  various disturbance scenarios
...
%DIFDELCMD < 		%%%
\DIFdel{We generated a realistic set of simulated networks based on the niche model~\mbox{%DIFAUXCMD
\citep{Williams2000,Stouffer2007} }\hspace{0pt}%DIFAUXCMD
using the function ``nichemodel'' within the Julia~\mbox{%DIFAUXCMD
\citep{Bezanson2017julia} }\hspace{0pt}%DIFAUXCMD
package }\emph{\DIFdel{BioEnergeticFoodWebs}}%DIFAUXCMD
\DIFdel{~\mbox{%DIFAUXCMD
\citep{bioenergfw,Delmas2017}}\hspace{0pt}%DIFAUXCMD
. 
		To capture a range of plausible network architectures, we simulated networks with sizes (S) ranging from 50 to 100 species (in steps of 10) and connectances (C) ranging from 0.02 to 0.18 (in steps of 0.04). 
		For full details and a comparison to empirical networks, see }\emph{\DIFdel{Appendix S2}}%DIFAUXCMD
\DIFdel{.
		We removed any biologically unlikely networks (}\emph{\DIFdel{Appendix S2}}%DIFAUXCMD
\DIFdel{) and replaced them with new simulated networks, repeating the process until we obtained 100 suitable networks in each combination of size and connectance.
		To make the simulated networks compatible with the Bayesian simulation framework, all networks were rendered acyclic following~\mbox{%DIFAUXCMD
\citet{Allesina2009functional} }\hspace{0pt}%DIFAUXCMD
and species topologically sorted from lowest to highest trophic level (}\emph{\DIFdel{Appendix S2}}%DIFAUXCMD
\DIFdel{). 
...
%DIFDELCMD < 		
%DIFDELCMD < 	%%%
\DIFdelend \subsection*{Defining species roles}
        ...
        Note, however, that this normalization does \DIFdelbegin \DIFdel{not necessarily }\DIFdelend \DIFaddbegin \emph{\DIFadd{not}} \DIFaddend control for differences due to degree \DIFdelbegin \DIFdel{or connectance as high- and low-degree species (or species in more- and less-connected networks) may }\DIFdelend \DIFaddbegin \emph{\DIFadd{per se}} \DIFadd{as high-degree species }\DIFaddend also tend to participate in different types of motifs \DIFdelbegin \DIFdel{.
        We address the potential associations between motif participation, degree, and connectance explicitly below}\DIFdelend \DIFaddbegin \DIFadd{than low-degree species~\mbox{%DIFAUXCMD
\citep{Cirtwill2021_inprep}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
    ...
    \subsubsection*{2) \DIFdelbegin \DIFdel{Motif participation }\DIFdelend \DIFaddbegin \DIFadd{Participation }\DIFaddend and other network properties}    
        \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <             %%%
\DIFdel{Properties which refer to different }\DIFdelend \DIFaddbegin \DIFadd{Different }\DIFaddend scales of network structure \DIFdelbegin \DIFdel{, including different amounts of direct and indirect interactions with the focal species, }\DIFdelend \DIFaddbegin \DIFadd{(e.g., local and meso-scale) }\DIFaddend are not independent. 
        To identify correlations between global network structure \DIFdelbegin \DIFdel{(i.e., the whole network) }\DIFdelend and participation in particular motifs\DIFdelbegin \DIFdel{(a meso-scale description of species roles)}\DIFdelend , we fit a series of LMs relating motif participation to network size, connectance, and their interaction (Appendix \emph{S3.2}).
        We fit a similar set of  LMMs relating motif participation to in-degree or trophic level (\DIFdelbegin \DIFdel{i.e., local measures of a species' role; }\DIFdelend Appendix \emph{S3.2}).
    ...
    \subsubsection*{3) \DIFdelbegin \DIFdel{Network mean persistence }\DIFdelend \DIFaddbegin \DIFadd{Persistence }\DIFaddend and motif profiles}
...
\section*{Results}
    ...
    \subsection*{Species \DIFdelbegin \DIFdel{persistence and }\DIFdelend motif participation \DIFaddbegin \DIFadd{and species persistence}\DIFaddend } 
    % Coefficients changed slightly with network-level random effect, but conclusions similar
       ...
       Persistence was not significantly related to the proportion of the three-species chain motif (Figure~\ref{fig:prop_lmer_all}; upper lines in each panel).
        ...
        At high levels of disturbance, a species' persistence decreased as the proportion of the three-species chain \DIFdelbegin \DIFdel{in its motif profile increased.
        Persistence }\DIFdelend \DIFaddbegin \DIFadd{increased, but persistence }\DIFaddend increased as the proportion of any other motif increased (Figure~\ref{fig:prop_lmer_all}; lower lines in each panel). 
        This means that increased disturbance strengthened the effects of the three-species chain motif and changed the direction of the effect of the apparent competition motif  (\DIFdelbegin \DIFdel{Figure~\ref{fig:prop_lmer_all}, }\DIFdelend Table S1, \emph{Appendix S4}).
    ...
    \DIFdelbegin \subsubsection*{\DIFdel{Consistency in effects of motif participation on species persistence}}
    %DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Consistency in effects of motif participation on species persistence}}
\DIFaddend 
    ...
    \subsection*{Network \DIFdelbegin \DIFdel{mean persistence and }\DIFdelend motif profiles \DIFaddbegin \DIFadd{and network mean persistence}\DIFaddend }
        ...
        Average persistence was higher in networks with higher proportions of the \DIFdelbegin \DIFdel{three-species }\DIFdelend chain, direct competition, and apparent competition motifs (Table S4, \emph{Appendix S8}). 
    ...
    \subsection*{\DIFdelbegin \DIFdel{Motif }\DIFdelend \DIFaddbegin \DIFadd{Network motif }\DIFaddend profiles and \DIFaddbegin \DIFadd{global }\DIFaddend network properties}
...
\section*{Discussion}
    ...
    The relationship between participation in the omnivory motif and persistence connects motif participation with both in-degree and trophic level\DIFdelbegin \DIFdel{(STL)}\DIFdelend .
    ...
    Motifs can thus act as a tool for synthesizing the information provided by\DIFaddbegin \DIFadd{, and interactions among, }\DIFaddend other measures of a species' place in its community\DIFdelbegin \DIFdel{and for understanding the interactions among these measures}\DIFdelend .
    ...
    \DIFdelbegin \DIFdel{For apparent competition , there were strong and opposing correlations }\DIFdelend \DIFaddbegin \DIFadd{Participation in the apparent competition motif was strongly negatively correlated }\DIFaddend with in-degree and \DIFdelbegin \DIFdel{trophic level but the relationship between apparent competition and persistence only matches the trends for in-degree.
    This suggests that some other factor is over-riding }\DIFdelend \DIFaddbegin \DIFadd{shows a similar relationship to persistence.
    However, participation in the apparent competition motif was also strongly and positively correlated with trophic level, and the negative effect of high trophic levels on persistence is not reflected in }\DIFaddend the relationship between \DIFdelbegin \DIFdel{apparent competition and trophic level}\DIFdelend \DIFaddbegin \DIFadd{persistence and participation in the apparent competition motif}\DIFaddend .
    ...
    In fact, a network's motif profile had a stronger relationship to mean persistence compared to \DIFdelbegin \DIFdel{both network size and }\DIFdelend \DIFaddbegin \DIFadd{network size or }\DIFaddend connectance. 
    ...
    Global-scale properties may \DIFdelbegin \DIFdel{therefore indirectly affect }\DIFdelend \DIFaddbegin \DIFadd{thus  have stronger effects on }\DIFaddend network persistence by changing meso-scale properties \DIFdelbegin \DIFdel{, such as the distribution of motifs}\DIFdelend \DIFaddbegin \DIFadd{than directly}\DIFaddend .
    ...
    Supporting this possibility, species participate more frequently in the omnivory motif in high-connectance networks \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \emph{\DIFadd{and}} \DIFaddend the relationship between persistence and participation in the omnivory motif is more strongly positive (Fig.~\ref{fig:density_prop_C}) as in~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{McLeod2021}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{McLeod2021}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \bibliographystyle{vancouver} 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliographystyle{ecollett} 
\DIFaddend \bibliography{anna_bib_new} % Abbreviate journal titles.
