...
\section{Participation and network properties} [not updated yet]
    ...
    \begin{table}[hb!]
        \centering
        \caption{Motif participation varies with in-degree and trophic level. Here we show the coefficients for models relating the frequency of each motif in a species' participation vector to in-degree or trophic level (8 models in total). [[Random effect?]]
...
\section{Persistence and global network properties} [updated]
    ...
    Connectance had a smaller, but significant, effect on consumer persistence than disturbance (Figs.~\ref{fig:lm_CS}, \ref{fig:heatmap_CS}, Table~\ref{tab:per_vs_SC}).
    ...
    \begin{table}[hb!]
        \caption{Mean persistence of consumers within a network depends on connectance (C) and the level of disturbance to basal resources (D), but not network size (S) or interactions between global properties. Here we show the effect sizes ($\beta$) and $p$-values for the general linear model described above. The model was fit using a binomial error distribution and logic link function. All predictors were centered and scaled before fitting the model;. The level of disturbance corresponds to basal resources having a probability of extinction ranging between $0.1 - 0.5$ (see \emph{Methods}). McFadden's pseudo-$R^2$=0.966.
        }
        \label{tab:per_vs_SC}
        \centering
        \begin{tabular}{c|c c |}
            ...
            \hline
            Intercept & 0.326 & \textless0.001 \\
            Size & -7.21$\times10^{-4}$  & 0.964 \\
            Connectance & -6.12$\times10^{-2}$& \textless0.001 \\
            Disturbance & -0.596 & \textless0.001 \\
