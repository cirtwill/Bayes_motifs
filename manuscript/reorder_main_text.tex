...
\section*{Introduction} %New
    ...
    %This can affect the influence of motif frequencies on network stability, as it has been demonstrated that omnivory may appear stabilizing dependent on interactions with other species \citep{Kratina2012}. 
...
%\subsection*{Here we test...}
   Here, we are interested in whether a consumer's extinction risk of going secondarily extinct depends on 1) the motifs in which the species participates, 2) the motif profile of the full food web and 3) the severity of the disturbance to the basal level. %We address this in scenarios with various levels of disturbance targeting the basal level.
    In addition, we want to understand how the information provided by motifs is related to global and local food web properties with known relationships to extinction risk in order to better synthesize information provided by different scales of network structure.
...
\section*{Methods}
    ...
    \subsection*{Modelling secondary extinction using Bayesian networks}
        ...
        As consumers do not affect the extinction probabilities of their resources, Bayesian networks can only capture bottom-up effects of disturbances within networks. 
        However, Bayesian networks do capture the majority of secondary extinctions captured by dynamical models \citep{Eklof2013}.
        This implies that bottom-up effects drive most secondary extinctions in simulations, echoing the strong bottom-up effects observed in empirical studies~\citep{}.
	...
	% Appendix 3, with numbered subheadings.
        ...
        We are primarily interested in whether a consumer's extinction risk is related to its motif participation or the motif profile of the network and, secondarily, also wish to connect the information provided by motifs to global and local network properties that have known effects on extinction risk.
    	To address these questions, we tested 1) how species persistence varied with motif participation, 2) how motif participation varied with other network properties (in-degree and STL), 3) how network mean persistence (the average persistence of consumers) varied with network motif profiles, 4) how network motif profiles varied with global structural properties (S and C), and 5) how species persistence and network mean persistence varied with other network properties (in-degree, STL, S, C, and level of disturbance).
        ...
        \subsubsection*{1) Species persistence and motif participation}
            ...
            Our main question of interest is whether appearing in different motifs is associated with differences in extinciton risk. 
        ...
        \subsubsection*{2) Motif participation and other network properties}    
            ...
            Properties which refer to different scales of network structure (e.g., local, global, and meso-scale) are not independent. 
            This means that, to understand relationships between motif participation (a meso-scale network property) and extinction risk, we must also consider how motif participation varies with other properties known to affect extinction risk.
            To identify correlations between global network structure (i.e., the whole network) and participation in particular motifs, we fit a series of LMs relating motif participation to network size, connectance, and their interaction (Appendix \emph{S3.2}).
        ...
        \subsubsection*{3) Network mean persistence and motif profiles}
            ...
            As well as relating species-specific probabilities of extinction to species-specific motif participation, we are interested in whether the overall frequency of motifs in a network provides information about the overall probability of any species going extinct.
            Just as other whole-network properties (S, C, etc.) are known to affect the overall stability of a network, it is possible that the arrangement of interactions into different motifs could reflect different average probabilities of persistence across all consumers.
        ...
        \subsubsection*{4) Motif profiles and network properties}
            ...
            Just like motif participation, a network's motif profile is not likely to be independent of other levels of structure. 
            To formally test whether networks with the the same size and connectance have more similar motif profiles, we used a PERMANOVA~\citep{Anderson2001} of Bray-Curtis dissimilarity among motif profiles against network size, connectance, and their interaction (Appendix \emph{S3.4}). 
            In addition, to understand how motif profiles may vary with global network structure in more detail, we identify correlations between size, connectance, and particular motifs using a set of four LMs:
            \begin{equation}
