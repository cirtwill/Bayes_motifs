...
\section*{Introduction} %New
    ...
    A species' position within a food web (a network of who eats whom) is known to affect its ability to persist following the extinction of  another species in the network (i.e., secondary extinction)~\citep{Santos2021,curtsdotter2011robustness, dunne2009cascading, Eklof2006}.
    ...
    Unlike in-degree and trophic level, which focus only on direct prey of the focal species or on the lengths of paths between the focal species and basal resources, motif participation also incorporates information on the focal species' interaction partners.
    ...
    Here we focus on persistence after the loss of basal resources (i.e., bottom-up disturbances).
    ...
    The loss of diversity at the basal level typically causes declines in both abundance and richness of all types of consumers: herbivores, predators, parasitoids, etc. \citep{scherber2010bottom}.
    Species' motif participation may indicate \emph{which} consumers are more or less likely to go extinct following such a disturbance.
    ...
    Although Bayesian networks currently only simulate bottom-up disturbances (our focal interest), they nevertheless capture the vast majority of secondary extinctions obtained from dynamical models including top-down and bottom-up effects~\citep{Eklof2013}.
    ...
    Although there are 13 possible three-species motifs (the most commonly-used size for food web analyses)~\citep{Milo2004,Stouffer2007,Stouffer2012,Cirtwill2015a}, we focus on just four: the three-species chain, apparent competition, direct competition, and omnivory.
    These four motifs are by far the most common in empirical networks~\citep{Stouffer2007, Borrelli2015a, giling2019plant}.
    ...
    Moreover, these four motifs have clear ecological interpretations, which are lacking for the cyclical motifs.
...
\section*{Results}
    ...
    \subsection*{Network mean persistence and motif profiles}
        ...
        Average persistence was higher in networks with higher proportions of the apparent competition motif and lower in networks with higher proportions of the omnivory motif (Table S4, \emph{Appendix S8}). 
    ...
    \subsection*{Motif profiles and network properties}
        ...
        Considering each motif separately, the proportion of the omnivory motif in a network's motif profile increased significantly with increasing connectance (Fig.~\ref{motif_proportion_lms}, Table S5, \emph{Appendix S9}).
        The proportions of other motifs in a network's profile were not related to network size, connectance, or their interaction.
        Consistent with the results of these GLMMs, a PERMANOVA test suggested that a network's motif profile as a whole was related to network size and connectance.
        However, the assumption of equal variance across groups was violated for this test and the PERMANOVA results should therefore be considered indicative only.
