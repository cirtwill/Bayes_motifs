...
\section*{Introduction} %New
    ...
    How population decline or extinction of one species leads to loss of other species via direct and indirect effects (i.e., secondary extinctions) is a vibrant area of research \citep{Santos2021,curtsdotter2011robustness, dunne2009cascading, Eklof2006}.
    Several global (whole-network) as well as local (single-species) properties are known to influence how species respond to disturbances. 
%network functioning \citep{Petchey2002}, stability \citep{Allesina2012} and robustness to disturbances \citep{Dunne2002, Eklof2006}. 
    Two well-known, influential global properties are network size, the number of species present in the network, and network connectance, the number of realized feeding interactions.
    ...
    Highly-connected networks may be more resistant to secondary extinctions \citep{Dunne2002, Eklof2006} because consumers, on average, have access to a larger number of prey species. However, in a well-connected network a disturbance may spread more easily and cause more secondary extinctions due to the presence of many possible pathways.
    ...
    Species with high trophic levels (long paths to basal resources) and low in-degree (few prey) are generally at higher risks of secondary extinction \citep{binzer2011susceptibility, Eklof2006}. 
    However, in-degree and trophic level are both relatively coarse descriptions of a species' role in its food web~\citep{Cirtwill2018FoodWebs}. For example, a consumer with low trophic level may be at high risk of extinction if it consumes prey which are themselves very likely to be affected by disturbance,and a specialist may not necessarily have a high risk of extinction if it consumes a basal resource which is unlikely to be affected by disturbance.

    %\subsection*{3 Motif participation may similarly affect species' particular extinction risk}
    One way to incorporate more nuance into descriptions of how species fit into their communities, and provide information linking global-scale and local-scale properties, is to define species' roles based on their participation in different motifs -- unique combinations of small numbers of interacting species~\citep{Stouffer2007,Stouffer2012}. 
    Such motif profiles of species are easily calculated once we know the network structure.
    Importantly, these `motif participation' roles capture information about species' direct and indirect interactions, incorporating larger-scale network structure into species-level descriptions~\citep{Cirtwill2015a}. 
    ...
    This extra detail may be useful when predicting extinction risks of single species, but to our knowledge this has not yet been investigated. 
    However, at the network level, it has been shown that the frequencies of different motifs are associated with community stability \citep{prill2005dynamic, bascompte2005simple} and with changes to the composition of plant communities \citep{giling2019plant}.
    While these studies uncovered the potential influence of network-level motif frequencies on network stability, a disturbance may likely alter the network and the interactions between species in different ways. 
    This can affect the influence of motif frequencies on network stability, as it has been demonstrated that an unstable motif may appear stabilizing dependent on interactions with other species \citep{Kratina2012}. 

%\subsection*{Here we test...}
    Therefore, we are interested in whether a consumer's extinction risk depends on 1) the motifs in which the species participates and 2) the motif profile of the full food web, in scenarios with various levels of disturbance targeting the basal level.
    ...
    We model extinction probabilities using Bayesian network modeling \citep{Eklof2013, Haussler2020}
    Of the 13 three-species motifs that can occur in empirical food webs, four motifs are over-represented in empirical networks and are thought to contribute to stability; the three-species chain, omnivory, apparent competition, and direct competition \citep{Stouffer2007, Borrelli2015a, giling2019plant}.
...
\section*{Methods}
    ...
    \subsection*{Bayesian network framework}	Traditionally, there are two main approaches for studying secondary extinctions. First, there are topological models, based only on food web structure \citep{dunne2009cascading}. Here, extinctions only affect other species in the network once a consumer has lost all of its prey and therefore must go extinct. 
